...
class RAGEngine:
    ...
    def query(
        ...
        deterministic = self._build_deterministic_answer(
            question,
            chunks,
        )

        if deterministic is not None:
            answer, answer_mode = deterministic
    ...
    def query_stream(
        ...
        if deterministic is not None:
            answer, _ = deterministic

            def deterministic_stream() -> Iterator[str]:
                yield answer

            return deterministic_stream(), chunks
    ...
    def _build_deterministic_answer(
        ...
        """Build deterministic answers shared by query and query_stream.

        Returns (answer, mode) when a deterministic rule applies.
        Returns None when the request should continue to the LLM.
        """
        ...
        if director_question:
            director_name = self._extract_director_name(chunks)
        ...
        if self._is_annual_financial_question(
            ...
            if financial_value:
                ...
                return (
                    f"{financial_label} tahun {requested_year} sebesar "
                    f"{normalized_value}.",
                    "table",
                )
